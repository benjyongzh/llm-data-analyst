...
| `available_charts` | array<string> | Whitelisted chart types |
...
| **Data retrieval\*** | `db_url` (string), `entities` (object), `current_task_index` (integer) | `tasks[current].sql` (string, set), `tasks[current].result` (object, set to `{type: "data", content: chart_spec}`), `tasks[current].error` (string, set), `thought` (array<object>, append success/failure) |
