...
- Natural-language terms are mapped to canonical schema names through a per‑database semantic layer. Mappings live in a `semantic_mapping` table keyed by `db_connection_id` and `user_id` and are loaded into an in‑memory cache for fast lookups
